...
def create_gantt(
    ...
    """
    **Deprecated**, use [`plotly.express.timeline`][plotly.express.timeline] instead.
    ...
    df : array-like or list
        Input data for Gantt chart. Must be either a dataframe or a list. If dataframe, the columns must include
        'Task', 'Start' and 'Finish'. Other columns can be included and used for indexing. If a list, its elements
        must be dictionaries with the same required column headers: 'Task', 'Start' and 'Finish'.
    colors : str, list, dict, or tuple
        ...
        where a, b, and c belong to [0, 1]. If colors is a list, it must contain the valid color types aforementioned as
        its members. If a dictionary, all values of the indexing column must be keys in colors.
    index_col : str or float
        The column header (if df is a dataframe) that will function as the indexing column. If df is a list, index_col
        must be one of the keys in all the items of df.
    show_colorbar : bool
        Determines if colorbar will be visible. Only applies if values in the index column are numeric.
    show_hover_fill : bool
        Enables/disables the hovertext for the filled area of the chart.
    reverse_colors : bool
    ...
    >>> from plotly.figure_factory import create_gantt
    >>> # Make data for chart
    >>> df = [dict(Task="Job A", Start='2009-01-01', Finish='2009-02-30', Resource='Apple'),
    ...
    plotly.graph_objs._figure.Figure
        A Plotly figure object representing the Gantt chart.
    """
