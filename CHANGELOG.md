...
### Fixed
- Fix the error messages for adding a `secondary_y` trace to a subplot created without the `secondary_y` spec, and for an invalid `output_type` in `plotly.io.from_json`/`read_json`, which showed unformatted placeholders such as `{subplot_type}` and `({row}, {col})` [[#5772](https://github.com/plotly/plotly.py/pull/5772)], with thanks to @David-Wu1119 for the contribution!
