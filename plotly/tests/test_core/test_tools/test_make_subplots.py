...
@raises(Exception)
def test_subplot_specs_wrong_inner_type():
    fig = tls.get_subplots(specs=[{}])

@raises(Exception)
def test_subplot_specs_wrong_item_type():
    fig = tls.get_subplots(specs=[[('not', 'going to work')]])

@raises(Exception)
def test_subplot_specs_wrong_item_key():
    fig = tls.get_subplots(specs=[{'not': "going to work"}])
...
# @raises(Exception)
# def test_subplot_specs_underspecified2():
...
def test_subplot_specs_colspan_rowpan():
...
def test_start_cell_wrong_values():
    fig = tls.get_subplots(rows=2, columns=2, start_cell='not gonna work')
