    ...
    def inspect_module(module, prefix=""):
        for name in dir(module):
            ...
            if inspect.isclass(obj):
                full_name = (
                    f"plotly.graph_objects.{name}" if not prefix else f"{prefix}.{name}"
                )
...
def generate_class_page(class_name, class_obj, output_dir):
    ...
    if len(parts) > 2:
        ...
        if len(relative_parts) == 1:
            ...
            parent_dirs = [f"{part}-package" for part in parent_parts[:-1]] + [
                f"{parent_parts[-1]}-package"
            ]
...
def generate_package_index(package_name, classes, packages, output_dir):
    ...
    if package_name == "plotly.graph_objects":
        ...
        relative_parts = parts[2:]
        package_dirs = [f"{part}-package" for part in relative_parts[:-1]] + [
            f"{relative_parts[-1]}-package"
        ]
    ...
    package_classes = [
        ...
        and name.count(".") == package_name.count(".") + 1
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate MkDocs pages for plotly.graph_objects"
    )
