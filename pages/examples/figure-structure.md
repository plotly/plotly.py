...
![Generated Plot](./figure-structure_1.png)
...
* Each trace has one of more than 40 possible types (see below for a list organized by subplot type, including e.g. [`scatter`](../line-and-scatter/), [`bar`](../bar-charts/), [`pie`](../pie-charts/), [`surface`](../3d-surface-plots/), [`choropleth`](../choropleth-maps/) etc), and represents a set of related graphical marks in a figure. Each trace must have a `type` attribute which defines the other allowable attributes.
* Each trace is drawn on a single [subplot](../subplots/) whose type must be compatible with the trace's type, or is its own subplot (see below).
* Traces may have a single [legend](../legend/) entry, with the exception of pie and funnelarea traces (see below).
* Certain trace types support [continuous color, with an associated colorbar](../colorscales/), which can be controlled by attributes either within the trace, or within the layout when using the [coloraxis attribute](../colorscales/).
...
The second of the three top-level attributes of a figure is `layout`, whose value is referred to in text as "the layout" and must be a dict, containing attributes that control positioning and configuration of non-data-related parts of the figure such as:
  ...
  * Figure-wide defaults: [templates](../templates/), [fonts](../figure-labels/), colors, hover-label and modebar defaults
  * [Title](../figure-labels/) and [legend](../legend/) (positionable in container and/or paper coordinates)
  * [Color axes and associated color bars](../colorscales/) (positionable in paper coordinates)
  ...
  * Non-data marks which can be positioned in paper coordinates, or in data coordinates linked to 2d cartesian subplots:
    * `annotations`: [textual annotations with or without arrows](../text-and-annotations/)
    * `shapes`: [lines, rectangles, ellipses or open or closed paths](../shapes/)
    * `images`: [background or decorative images](../images/)
...
The most commonly-used kind of subplot is a [two-dimensional Cartesian subplot](../axes/). Traces compatible with these subplots support `xaxis` and `yaxis` attributes whose values must refer to corresponding objects in the layout portion of the figure. For example, if `xaxis="x"`, and `yaxis="y"` (which is the default) then this trace is drawn on the subplot at the intersection of the axes configured under `layout.xaxis` and `layout.yaxis`, but if `xaxis="x2"` and `yaxis="y3"` then the trace is drawn at the intersection of the axes configured under `layout.xaxis2` and `layout.yaxis3`. Note that attributes such as `layout.xaxis` and `layout.xaxis2` etc do not have to be explicitly defined, in which case default values will be inferred. Multiple traces of different types can be drawn on the same subplot.

X- and Y-axes support the `type` attribute, which enables them to represent [continuous values (`type="linear"`, `type="log"`)](../axes/), [temporal values (`type="date"`)](../time-series/) or [categorical values (`type="category"`, `type="multicategory`)](../bar-charts/). Axes can also be overlaid on top of one another to create [dual-axis or multiple-axis charts](../multiple-axes/). 2-d cartesian subplots lend themselves very well to creating ["small multiples" figures, also known as facet or trellis plots](../facet-plots/).

The following trace types are compatible with 2d-cartesian subplots via the `xaxis` and `yaxis` attributes:

* scatter-like trace types: [`scatter`](../line-and-scatter/) and [`scattergl`](../webgl-vs-svg/), which can be used to draw [scatter plots](../line-and-scatter/), [line plots and curves](../line-charts/), [time-series plots](../time-series/), [bubble charts](../bubble-charts/), [dot plots](../dot-plots/) and [filled areas](../filled-area-plots/) and also support [error bars](../error-bars/)
* [`bar`](../bar-charts/), [`funnel`](../funnel-charts/), [`waterfall`](../waterfall-charts/): bar-like trace types which can also be used to draw [timelines and Gantt charts](../gantt/)
* [`histogram`](../histograms/): an *aggregating* bar-like trace type
* [`box`](../box-plots/) and [`violin`](../box-plots/): 1-dimensional distribution-like trace types
* [`histogram2d`](../2D-Histogram/) and [`histogram2dcontour`](../2d-histogram-contour/): 2-dimensional distribution-like density trace types
* [`image`](../imshow/), [`heatmap`](../heatmaps/) and [`contour`](../contour-plots/): matrix trace types
* [`ohlc`](../ohlc-charts/) and [`candlestick`](../candlestick-charts/): stock-like trace types
* [`carpet`](../carpet-plot/): a special trace type for building [carpet plots](../carpet-plot/), in that other traces can use as subplots (see below)
* [`splom`](../splom/): multi-dimensional scatter plots which implicitly refer to many 2-d cartesian subplots at once.

### 3D, Polar, Ternary and Smith Trace Types and Subplots

Beyond 2D cartesian subplots, figures can include [three-dimensional cartesian subplots](../3d-charts/), [polar subplots](../polar-chart/), [ternary subplots](../ternary-plots/) and [smith subplots](../smith-charts/). The following trace types support attributes named `scene`, `polar`, `smith` or `ternary`, whose values must refer to corresponding objects in the layout portion of the figure i.e. `ternary="ternary2"` etc. Note that attributes such as `layout.scene` and `layout.ternary2` etc do not have to be explicitly defined, in which case default values will be inferred. Multiple traces of a compatible type can be placed on the same subplot.

The following trace types are compatible with 3D subplots via the `scene` attribute, which contains special [camera controls](../3d-camera-controls/):

* [`scatter3d`](../3d-scatter-plots/), which can be used to draw [individual markers](../3d-scatter-plots/), [3d bubble charts](../3d-bubble-charts/) and [lines and curves](../3d-line-plots/)
* [`surface`](../3d-surface-plots/) and [`mesh`](../3d-mesh/): 3d surface trace types
* [`cone`](../cone-plot/) and [`streamtube`](../streamtube-plot/): 3d vector field trace types
* [`volume`](../3d-volume-plots/) and [`isosurface`](../3d-isosurface-plots/): 3d volume trace types

The following trace types are compatible with polar subplots via the `polar` attribute:

* scatter-like trace types: [`scatterpolar` and `scatterpolargl`](../polar-chart/), which can be used to draw individual markers, [curves and filled areas (i.e. radar or spider charts)](../radar-chart/)
* [`barpolar`](../wind-rose-charts/): useful for [wind roses](../wind-rose-charts/) and other polar bar charts

The following trace types are compatible with ternary subplots via the `ternary` attribute:

* [`scatterternary`](../ternary-plots/), which can be used to draw individual markers, [curves and filled areas](../ternary-contour/)

The following trace types are compatible with smith subplots via the `smith` attribute:

* [`scattersmith`](../smith-charts/), which can be used to draw individual markers, curves and filled areas

### Map Trace Types and Subplots

Figures can include two different types of map subplots: [geo subplots for outline maps](../map-configuration/) and [tile-based maps](../tile-map-layers/). The following trace types support attributes named `geo` or `map`, whose values must refer to corresponding objects in the layout i.e. `geo="geo2"` etc.  Note that attributes such as `layout.geo2` and `layout.map` etc do not have to be explicitly defined, in which case default values will be inferred. Multiple traces of a compatible type can be placed on the same subplot.

The following trace types are compatible with geo subplots via the `geo` attribute:

* [`scattergeo`](../scatter-plots-on-maps/), which can be used to draw [individual markers](../scatter-plots-on-maps/), [line and curves](../lines-on-maps/) and filled areas on outline maps
* [`choropleth`](../choropleth-maps/): [colored polygons](../choropleth-maps/) on outline maps

The following trace types are compatible with tile map subplots via the `map` attribute:

* [`scattermap`](../tile-scatter-maps/), which can be used to draw [individual markers](../tile-scatter-maps/), [lines and curves](../lines-on-tile-maps/) and [filled areas](../filled-area-tile-maps/) on tile maps
* [`choroplethmap`](../tile-county-choropleth/): colored polygons on tile maps
* [`densitymap`](../tile-density-heatmaps/): density heatmaps on tile maps
...
* [`pie`](../pie-charts/) and [`funnelarea`](../waterfall-charts/): one-level part-to-whole relationships with legend items
* [`sunburst`](../sunburst-charts/) and [`treemap`](../treemaps/): hierarchical multi-level part-to-whole relationships
* [`parcoords`](../parallel-coordinates-plot/) and [`parcats`](../parallel-categories-diagram/): continuous and categorical multidimensional figures with [parallel coordinates](../parallel-coordinates-plot/) and [parallel sets](../parallel-categories-diagram/)
* [`sankey`](../sankey-diagram/): [flow diagrams](../sankey-diagram/)
* [`table`](../table/): [text-based tables](../table/)
* [`indicator`](../indicator/): big numbers, [gauges](../gauge-charts/), and [bullet charts](../bullet-charts/)

### Carpet Trace Types and Subplots

Certain trace types use [traces of type `carpet` as a subplot](../carpet-plot/). These support a `carpet` attribute whose value must match the value of the `carpet` attribute of the `carpet` trace they are to be drawn on. Multiple compatible traces can be placed on the same `carpet` trace.

The following trace types are compatible with `carpet` trace subplots via the `carpet` attribute:

* [`scattercarpet`](../carpet-scatter/), which can be used to draw individual markers, curves and filled areas
* [`contourcarpet`](../carpet-plot/)
